...
SELECT *
    FROM champs_il t right OUTER JOIN structure_info_lin_cc r ON (r.nom_champ_ref = t.nom_champ_test);
...
INSERT INTO erreurs_champs_il SELECT * FROM (SELECT *
    FROM  champs_il t left OUTER JOIN structure_info_lin_cc r ON (r.nom_champ_ref = t.nom_champ_test)) as champs_invalid;
...
--copy (Select * from erreurs_champs_il) to 'D:\pg_data\erreurs_structure_info_lin_cc.txt'
