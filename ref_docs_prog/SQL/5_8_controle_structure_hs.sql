;
--champs présents:
DROP table champs_hs;
CREATE table champs_hs as
SELECT column_name as nom_champ_test, data_type as type_champ_test
FROM information_schema.columns
WHERE table_name = 'habillage_surf' 
...
DROP table erreurs_champs_hs;
CREATE table erreurs_champs_hs as
SELECT *
    FROM champs_hs t right OUTER JOIN structure_habillage_surf r ON (r.nom_champ_ref = t.nom_champ_test);

DELETE from erreurs_champs_hs
   WHERE nom_champ_test=nom_champ_ref and type_champ_test=type_champ_ref;

UPDATE erreurs_champs_hs
...
INSERT INTO erreurs_champs_hs SELECT * FROM (SELECT *
    FROM  champs_hs t left OUTER JOIN structure_habillage_surf r ON (r.nom_champ_ref = t.nom_champ_test)) as champs_invalid;
...
ALTER TABLE erreurs_champs_hs
ADD column HABILLAGE_SURF character varying;
UPDATE erreurs_champs_hs
    SET HABILLAGE_SURF = 'ERREURdg' where erreurs<>'id_map' or liste='id_map';

--select * from erreurs_champs_hs;
--copy (Select * from erreurs_champs_hs) to 'D:\pg_data\erreurs_structure_habillage_surf.txt'
