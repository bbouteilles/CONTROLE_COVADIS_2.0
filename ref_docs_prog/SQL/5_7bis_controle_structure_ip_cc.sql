...
SELECT *
    FROM champs_ip t right OUTER JOIN structure_info_pct_cc r ON (r.nom_champ_ref = t.nom_champ_test);
...
INSERT INTO erreurs_champs_ip SELECT * FROM (SELECT *
    FROM  champs_ip t left OUTER JOIN structure_info_pct_cc r ON (r.nom_champ_ref = t.nom_champ_test)) as champs_invalid;
...
--copy (Select * from erreurs_champs_ip) to 'D:\pg_data\erreurs_structure_info_pct_cc.txt'
