...
select st_geomfromtext(st_astext((st_dumprings(ST_Union(the_geom))).geom))as trous, st_area(st_geomfromtext(st_astext((st_dumprings(ST_Union(the_geom))).geom))) as trou_en_m² from public.zone_urba;
