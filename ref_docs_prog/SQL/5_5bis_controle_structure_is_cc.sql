...
SELECT *
    FROM champs_is t right OUTER JOIN structure_info_surf_cc r ON (r.nom_champ_ref = t.nom_champ_test);
...
INSERT INTO erreurs_champs_is SELECT * FROM (SELECT *
    FROM  champs_is t left OUTER JOIN structure_info_surf_cc r ON (r.nom_champ_ref = t.nom_champ_test)) as champs_invalid;
...
--copy (Select * from erreurs_champs_is) to 'D:\pg_data\erreurs_structure_info_surf_cc.txt'
